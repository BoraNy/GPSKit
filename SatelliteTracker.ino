    ...
    if (ss.available() > 0)
    {
        ...
        if (totalGPGSVMessages.isUpdated())
        {
            ...
            if (totalMessages == currentMessage)
            {
                ...
                display.print("SATELLITE TRACKER");

                /* Display Satellite Number */
                display.setTextColor(WHITE);
                display.setCursor(80, 11);
                display.print(satellite_id[0]);
                display.print(',');
                display.print(satellite_id[1]);
                display.print(',');
                display.print(satellite_id[2]);

                display.setCursor(80, 21);
                display.print(satellite_id[3]);
                display.print(',');
                display.print(satellite_id[4]);
                display.print(',');
                display.print(satellite_id[5]);

                display.setCursor(80, 31);
                display.print(satellite_id[6]);
                display.print(',');
                display.print(satellite_id[7]);
                display.print(',');
                display.print(satellite_id[8]);

                display.setCursor(80, 41);
                display.print(satellite_id[9]);
                display.print(',');
                display.print(satellite_id[10]);
                display.print(',');
                display.print(satellite_id[11]);
