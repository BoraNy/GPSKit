void noReceptionSignal(void) {
  ...
  if ((gps.location.lat() == 0) && (gps.location.lng() == 0)
    ...
    for (int step = 0; step < 3; step++) {
      ...
      display.print(F("SIGNAL RECEPTION TOOK"));
      display.setCursor(40, 11);
      display.print(F("TOO LONG"));
      display.setCursor(13, 39);
      display.print(F("CHECK THE ANTENNA"));
      ...
      if (gps_var.show_question_mark) {
        ...
        display.print(F("ARE YOU INDOOR?"));
      } else {
...
void GPSMenu(void) {
  ...
  if ((gps.location.lat() == 0) && (gps.location.lng() == 0) && (gps.time.hour() == 0) && (gps.time.minute() == 0)) {
