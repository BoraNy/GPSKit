...
void loop() {
  switch (menu.tab) {
    case 0:
      ShowMenuIcon(GPS_ICON); /* GPS */
      GPSMenu();
      ButtonIO();
      break;

    case 1:
      ShowMenuIcon(SATELLITE_ICON); /* Satellites Tracker */
      SatelliteTracker();
      ButtonIO();
      break;

    case 2:
      ShowMenuIcon(DEVIATION_MAP_ICON); /* Deviation Map */
      DeviationMap();
      ButtonIO();
      break;

    case 3:
      ShowMenuIcon(COMPASS_ICON); /* Compass Menu */
      CompassMenu();
      ButtonIO();
      break;

    case 4:
      ShowMenuIcon(ALTIMETER_ICON); /* Altitude Graph */
      AltitudeMenu();
      ButtonIO();
      break;

    case 5:
      ShowMenuIcon(HARDWARE_ICON); /* Hardware Info */
      InfoMenu();
      ButtonIO();
      break;
  }
